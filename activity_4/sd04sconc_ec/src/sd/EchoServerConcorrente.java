...
public class EchoServerConcorrente {
	...
	// um servidor TCP: ver outro exemplo no livro
        ...
        if (server.isClosed())
            System.err.println("\t Socket de servico foi fechado.");
        else{
            e.printStackTrace();
...
class ServiceManager extends Thread {
    ...
    public void adminService() {
        try {
            ...
            // Ler qual a operacao de administracao
            data.close();
            // Se a operacao e parar o servico
            adminSocket.close();
            serviceSocket.close(); // Termina o servico
        }
        catch (Exception e) {
            e.printStackTrace();
        }
